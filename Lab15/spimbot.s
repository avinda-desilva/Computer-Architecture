...
ih_done:
  ...
  puzzle_request:
  	li		$t2,  30		# puzzles needed to be solved =  10
  	bge		$s4, $t2, start_paint	# if  <  then
  	la		$t1, puzzle_flag		#
  	li		$t0, 0 		# 0= 0
  	sw		$t0, 0($t1)		#
  	la		$t0, puzzle		# Load puzzle
  	sw		$t0, REQUEST_PUZZLE($zero)		# Store puzzle in request
  check_flag:
  	la		$t1, puzzle_flag		#
  	lw		$t1, 0($t1)		#
  	beq   $t1, $0, check_flag
  solve_puzzle:
  	...
  	li  $a1, 0        # row = 0
  	li  $a2, 0        # col = 0
  	la  $a3, puzzle  # puzzle = puzzle1
  	jal solve
  	la	$t1, puzzle		# Load solved puzzle
  	sw	$t1, SUBMIT_SOLUTION($zero)		#
  	add $s4, $s4, 1
  	j puzzle_request
  start_paint:
  	li 			$s1, 0				# Save angle to savee registers
  	li 			$t1, 1
    #sw      $t1, PICKUP_POWERUP($0)    #
  	sw			$t1, SWITCH_MODE($0)		# Enable Paint Brush
    #sw			$0, USE_POWERUP($0)		# Enable Paint Brush


    li $t0, 10
    sw $t0, VELOCITY($zero)			# Set velocity to 10


    # get_more_puzzle:
    #     li		$t2,  5 		# puzzles needed to be solved =  10
    #   	bge		$s4, $t2, start_paint	# if  <  then
    #   	la		$t1, puzzle_flag		#
    #   	li		$t0, 0 		# 0= 0
    #   	sw		$t0, 0($t1)		#
...
interrupt_dispatch:            # Interrupt:
    ...
    bonk_interrupt:
    	sw 		$0, BONK_ACK
        #Fill in your code here
    		sw $s1, ANGLE($zero) # Set angle
    		sw $zero, ANGLE_CONTROL($zero) # Control = 0 to make sure it is relative
    		add $s1, $s1, 1 # Increase angle by 1
    		rem $s1, $s1, 360 # To make sure it is an integer between 0 and 360
        li $t1, 10
    		sw $t1, VELOCITY($zero)
        lw      $t1, GET_PAINT_BUCKETS
        blt     $t1, 35, use_power
    ...
    request_puzzle_interrupt:
    	...
    	la		$t0, puzzle_flag		# Get pointer to flag
    	li 		$t1, 1
    	sw		$t1, 0($t0)		# Store 1 into the flag
    	#Fill in your code here
    	j	interrupt_dispatch
    timer_interrupt:
    	sw 		$0, TIMER_ACK
